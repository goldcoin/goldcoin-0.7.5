...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
	...
	CBigNum bnResult;
	if (!hardForkedJuly && !hardForkedNovember) {
...
{
	return std::max(cPeerBlockCounts.mode(), Checkpoints::GetTotalBlocksEstimate());
}
...
bool CBlock::CheckBlock(CNode* pfrom) const
{
	...
	// Check Timestamp
	if (GetBlockTime() > GetAdjustedTime() + 2 * 60 * 60 && nBestHeight <= octoberFork) {
		return error("CheckBlock() : block timestamp too far in the future 1");
	} else if (GetBlockTime() > GetAdjustedTime() + 45 && nBestHeight > octoberFork) {
		...
		if (!pfrom || (pfrom->addr.ToString().find("local") != std::string::npos || pfrom->addr.ToString().find("127.0.0.") != std::string::npos)) { //If its a local block
			...
			if (boost::shared_ptr<CBlock> theBlock = getPreviousBlock(&nBlock, 4)) // 4 = 5
				if (QDateTime::fromTime_t(theBlock->GetBlockTime()).secsTo(QDateTime::fromTime_t(GetBlockTime())) < (60 * 10)) {
					...
					return error("CheckBlock() : block timestamp too far in the future 3");
				}
		} else {
			return error("CheckBlock() : block timestamp too far in the future 4");
		}
	}
...
bool CBlock::AcceptBlock()
{
	...
	if (checkpointBlockNum <= nHeight && checkpointBlockNum != -1) {
		CBlock nBlock;
		nBlock.ReadFromDisk(mapBlockIndex[hashPrevBlock]);
		if (CBlock* theBlock = getPreviousBlock(&nBlock, 9).get())
			Checkpoints::addCheckpoint(nHeight - 10, theBlock->GetHash());
	}
...
boost::shared_ptr<CBlock> getPreviousBlock(CBlock* block, int64 numBlocksBefore) {
	int64 curBlocks = numBlocksBefore;
	//Copy block so we don't double-deallocate
	boost::shared_ptr<CBlock> curBlock = boost::make_shared<CBlock>(*new CBlock(*block));

	while (curBlocks) {
		if (mapBlockIndex.count(curBlock->hashPrevBlock)) {
			CBlock nBlock;
			if (nBlock.ReadFromDisk(mapBlockIndex[curBlock->hashPrevBlock])) {
				curBlock = boost::make_shared<CBlock>(*new CBlock(nBlock));
			}
		} else if (mapOrphanBlocks.count(curBlock->hashPrevBlock)) {
			curBlock = boost::make_shared<CBlock>(*new CBlock(*mapOrphanBlocks[curBlock->hashPrevBlock]));
		} else {
			error("getPreviousBlock() : Could not find ancestor of : %s", curBlock->GetHash().ToString().c_str());
			return boost::make_shared<CBlock>();
		}
		curBlocks--;
	}
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
	...
	*/
	if(nBestHeight > octoberFork)
	if(boost::shared_ptr<CBlock> fifthPreviousBlock = getPreviousBlock(pblock, 5))
	if (QDateTime::fromTime_t(fifthPreviousBlock->GetBlockTime()).secsTo(QDateTime::fromTime_t(pblock->GetBlockTime())) < (60 * 10)) {
		...
		printf("ProcessBlock() : Time between last 5 blocks is: %d", QDateTime::fromTime_t(fifthPreviousBlock->GetBlockTime()).secsTo(QDateTime::fromTime_t(pblock->GetBlockTime())));
	}
