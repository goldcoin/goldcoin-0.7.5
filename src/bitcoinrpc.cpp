...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop GoldCoin(GLD) server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "GoldCoin(GLD) server has now stopped running!";
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new GoldCoin (GLD) address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current GoldCoin (GLD) address for receiving payments to this account.");
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: GoldCoin (GLD) address and we have full public key:
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; GoldCoin(GLD) server stopping, restart to run with encrypted wallet";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "GoldCoin (GLD) is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "GoldCoin (GLD) is downloading blocks...");
