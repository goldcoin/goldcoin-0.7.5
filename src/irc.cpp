...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            ...
            // randomly join #gldcoin00-#gldcoin99
            int channel_number = GetRandInt(100);
            channel_number = 0; // GLD: for now, just use one channel
            Send(hSocket, strprintf("JOIN #gldcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #gldcoin%02d\r", channel_number).c_str());
        }
